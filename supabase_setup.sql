...
-- =======================================================
-- REFERRAL / COUPON CODE ACCESS GATE SCHEMAS (FIXED TYPES)
-- =======================================================

-- Drop pre-existing tables if they were created with incompatible UUID types
DROP TABLE IF EXISTS user_subject_access CASCADE;
...
CREATE TABLE IF NOT EXISTS referral_codes (
  ...
  subject_key TEXT DEFAULT NULL, -- NULL or 'all' for universal, or 'science', 'mathematics', 'history_pol_sc', 'geo_eco'
...
-- 2. Create user_subject_access table
CREATE TABLE IF NOT EXISTS user_subject_access (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  user_id BIGINT REFERENCES users(id) ON DELETE CASCADE NOT NULL,
  subject_key TEXT NOT NULL, -- 'science', 'mathematics', 'history_pol_sc', 'geo_eco', 'all'
  access_type TEXT DEFAULT 'referral', -- 'referral' | 'purchase' | 'admin'
  referral_code_id BIGINT REFERENCES referral_codes(id) ON DELETE SET NULL,
  granted_at TIMESTAMPTZ DEFAULT NOW(),
  CONSTRAINT unique_user_subject UNIQUE(user_id, subject_key)
);
...
-- Allow public policies for applet / Express server backend access
DROP POLICY IF EXISTS "Allow public select referral_codes" ON referral_codes;
CREATE POLICY "Allow public select referral_codes" ON referral_codes FOR SELECT USING (true);
...
DROP POLICY IF EXISTS "Allow public update referral_codes" ON referral_codes;
CREATE POLICY "Allow public update referral_codes" ON referral_codes FOR UPDATE USING (true);

DROP POLICY IF EXISTS "Allow public delete referral_codes" ON referral_codes;
CREATE POLICY "Allow public delete referral_codes" ON referral_codes FOR DELETE USING (true);


DROP POLICY IF EXISTS "Allow public select user_subject_access" ON user_subject_access;
CREATE POLICY "Allow public select user_subject_access" ON user_subject_access FOR SELECT USING (true);

DROP POLICY IF EXISTS "Allow public insert user_subject_access" ON user_subject_access;
CREATE POLICY "Allow public insert user_subject_access" ON user_subject_access FOR INSERT WITH CHECK (true);

DROP POLICY IF EXISTS "Allow public update user_subject_access" ON user_subject_access;
CREATE POLICY "Allow public update user_subject_access" ON user_subject_access FOR UPDATE USING (true);

DROP POLICY IF EXISTS "Allow public delete user_subject_access" ON user_subject_access;
CREATE POLICY "Allow public delete user_subject_access" ON user_subject_access FOR DELETE USING (true);
